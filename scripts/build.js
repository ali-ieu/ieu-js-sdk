...
async function build(target) {
    ...
    if (buildTypes && pkg.types) {
        ...
        if (extractorResult.succeeded) {
            // concat additional global.d.ts to rolled-up dts
            const additionalDts = path.resolve(pkgDir, 'global.d.ts')
            if (await fs.exists(additionalDts)) {
                // get exists dts
                const dtsPath = path.resolve(pkgDir, pkg.types)
                // append
                await fs.writeFile(dtsPath, '\n' + (await fs.readFile(additionalDts, 'utf-8')), {
                    flag: 'a',
                })
            }
